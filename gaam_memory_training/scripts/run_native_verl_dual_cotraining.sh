...
if [[ -n "${MAX_MEMORY_CHUNK_CHARS}" ]]; then
  ARGS+=(--max_memory_chunk_chars "${MAX_MEMORY_CHUNK_CHARS}")
fi
...
if [[ "${ALLOW_STATIC_INCREMENTAL_SCAFFOLD}" == "1" || "${ALLOW_STATIC_INCREMENTAL_SCAFFOLD}" == "True" || "${ALLOW_STATIC_INCREMENTAL_SCAFFOLD}" == "true" ]]; then
  ARGS+=(--allow_static_incremental_scaffold)
fi
...
if [[ "${REQUIRE_HF_CHECKPOINT}" == "0" || "${REQUIRE_HF_CHECKPOINT}" == "False" || "${REQUIRE_HF_CHECKPOINT}" == "false" ]]; then
  ARGS+=(--no_require_hf_checkpoint)
fi
